...
def main():
    ...
    experiments = ['Dot', 'Trigonometric', 'XOR']
    for experiment in experiments:
        if experiment == 'XOR':
            result_dir = './results/toy_xor/'
        elif experiment == 'Trigonometric':
        ...
        for split in range(cv):
            ...
            # # if experiment == 'Dot':
            # x_train = results[f'{split}']['x_train']
            # x_train_prod = x_train[:, :2] * x_train[:, 2:]
            # y_train_ctx = results[f'{split}']['c_emb_train'].reshape(6000, 2)
            # y_train = (results[f'{split}']['c_train']>0.5).reshape(6000,)
            # plt.figure()
            # # plt.scatter(x_train_prod[:, 0], x_train_prod[:, 1])
            # # plt.scatter(y_train_ctx[:, 0], y_train_ctx[:, 1])
            # plt.scatter(y_train_ctx[y_train, 0], y_train_ctx[y_train, 1])
            # plt.scatter(y_train_ctx[~y_train, 0], y_train_ctx[~y_train, 1])
            # plt.show()
