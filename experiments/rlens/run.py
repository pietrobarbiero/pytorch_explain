...
def main():
    ...
    for dataset in datasets:
        for fold in folds:
            ...
            model = DeepConceptReasoner(rule_learner, reasoner, scorer, concept_names, class_names, lr, epochs)
            model.fit(c[train_mask], c_emb[train_mask], y1h[train_mask])
            y_test_pred_reasoner, y_test_pred_learner = model.predict(c[test_mask], c_emb[test_mask], y1h[test_mask])
