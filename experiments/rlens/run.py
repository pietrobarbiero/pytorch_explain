...
from experiments.rlens.model import get_rule_learner, DeepConceptReasoner, get_reasoner, get_scorer
...
def main():
    ...
    for dataset in datasets:
        for fold in folds:
            ...
            y_test_pred_reasoner, y_test_pred_learner, c_pred_reasoner, y_pred_bool, y_pred_refined = model.predict(c[test_mask], c_emb[test_mask])
            ...
            test_accuracy_refined = (y_pred_refined > 0.5).eq(y1h[test_mask]).sum().item() / (y1h[test_mask].size(0) * y1h[test_mask].size(1))
            print(f'Test accuracy refined: {test_accuracy_refined:.4f}')
            ...
            res1 = pd.DataFrame([[model.learnt_rules_, test_accuracy_reasoner, fold, 'VL', dataset]], columns=['rules', 'accuracy', 'fold', 'model', 'dataset'])
            res2 = pd.DataFrame([[model.learnt_rules_, test_accuracy_refined, fold, 'DCR+', dataset]], columns=['rules', 'accuracy', 'fold', 'model', 'dataset'])
            res3 = pd.DataFrame([[None, test_accuracy_cem, fold, 'CEM', dataset]], columns=['rules', 'accuracy', 'fold', 'model', 'dataset'])
            res4 = pd.DataFrame([[None, test_accuracy_learner, fold, 'LENs', dataset]], columns=['rules', 'accuracy', 'fold', 'model', 'dataset'])
            res5 = pd.DataFrame([[model.learnt_rules_, test_accuracy_bool, fold, 'TNorm', dataset]], columns=['rules', 'accuracy', 'fold', 'model', 'dataset'])

            if len(results) == 0:
                results = res1
                results = pd.concat((results, res2, res3, res4, res5))
            else:
                results = pd.concat((results, res1, res2, res3, res4, res5))
