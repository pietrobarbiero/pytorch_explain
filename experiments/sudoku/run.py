...
def main():
    ...
    X, c, manifolds, y = sudoku(n)

    X = torch.stack(X, dim=0).float()
    c, manifolds, y = (torch.tensor(i) for i in (c, manifolds, y))


    c_train = F.one_hot(c.long()).float()
    y_train = y.float()

    train_data = TensorDataset(X, c_train, y_train)
    test_data = TensorDataset(X, c_train, y_train)
    ...
    model = SudokuRelationalDCR(emb_size=10, manifolds = manifolds, manifold_arity=n, num_classes=n)
